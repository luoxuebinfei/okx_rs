...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let secret_key =
        env::var("OKX_SECRET_KEY").unwrap_or_else(|_| "your_secret_key".to_string());
    let passphrase =
        env::var("OKX_PASSPHRASE").unwrap_or_else(|_| "your_passphrase".to_string());
    ...
    if let Some(t) = ticker.first() {
        println!(
            "BTC-USDT last={} bid={} ask={}",
            t.last, t.bid_px, t.ask_px
        );
    }
