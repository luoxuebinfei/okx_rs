...
## 项目概览
- Rust 实现的高性能 OKX REST/WebSocket SDK，接口路径与参数完全来自官方文档（https://www.okx.com/docs-v5/），并对照官方 `python-okx` 行为实现。
- Workspace 拆分：`okx-core`（配置/签名/通用类型）、`okx-rest`（REST 客户端）、`okx-ws`（WebSocket 客户端）、`okx-py`（PyO3 绑定，含同步/异步/WS）。
...
- `crates/okx-rest`：`OkxRestClient` + `AccountApi` / `TradeApi` / `FundingApi` / `MarketApi` / `PublicApi`，封装私有/公共请求。
- `crates/okx-ws`：`WsClient`（基础）与 `ReconnectingWsClient`（自动重连），`Channel`/`WsMessage` 模型。
- `crates/okx-py`：Python 客户端（`OkxClient`、`AsyncOkxClient`、`WsClient`）与类型导出，基于 PyO3。
- 统一脚本：`justfile`（fmt/clippy/test/py-test/typecheck/ci/cov）。
